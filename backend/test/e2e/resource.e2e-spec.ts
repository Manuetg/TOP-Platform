...
describe('Resource endpoint', () => {
  ...
  beforeAll(async () => {
    const module = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .overrideProvider(RESOURCE_REPOSITORY).useValue({ findByIdAndBusinessId: (id: string, businessId: string): Promise<Resource | null> => Promise.resolve(resources.find((item) => item.id === id && item.businessId === businessId) ?? null), findByBusinessAndCode: jest.fn(), create: jest.fn() }).compile();

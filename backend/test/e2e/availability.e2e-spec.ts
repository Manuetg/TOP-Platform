...
describe('Availability endpoint', () => {
  let app: INestApplication; let resourceStatus = ResourceStatus.ACTIVE; let bookingConflict = false; let blockConflict = false;
  beforeAll(async () => {
    const module = await Test.createTestingModule({ imports: [AppModule] })
      .overrideProvider(BUSINESS_REPOSITORY).useValue({ findById: (id: string) => Promise.resolve(id === businessId ? { status: BusinessStatus.ACTIVE } : null), create: jest.fn(), list: jest.fn(), update: jest.fn() })
  ...
  beforeEach(() => { resourceStatus = ResourceStatus.ACTIVE; bookingConflict = false; blockConflict = false; });

...
const securityBusiness = Business.create({ id: businessId, businessNumber: null, name: 'Security Business', legalName: null, taxId: null, timezone: 'America/Asuncion', currency: 'PYG', status: BusinessStatus.ACTIVE, createdAt: new Date(), updatedAt: new Date() });
...
describe('Protección JWT y membresía', () => {
  ...
  it('protege el catálogo Rate Plan y permite lectura a VIEWER con pricing.read', async () => {
    const endpoint = `/api/businesses/${businessId}/rate-plans`;
    await request(app.getHttpServer()).get(endpoint).expect(401);
    const token = await bearer();
    await request(app.getHttpServer()).get(endpoint).set('Authorization', `Bearer ${token}`).expect(403);
    memberships = [membership(businessId, MembershipRole.VIEWER)];
    await request(app.getHttpServer()).get(endpoint).set('Authorization', `Bearer ${token}`).expect(200, []);
  });

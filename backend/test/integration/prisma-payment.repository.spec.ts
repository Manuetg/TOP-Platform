...
describeWithPostgres('PrismaPaymentRepository with PostgreSQL', () => {
  ...
  it('persists the payment record and isolates tenant idempotency keys', async () => { const first = await fixture(); const second = await fixture(); const created = await repository.register(data(first.business.id, first.booking.id, 'same-key'), 100); await repository.register(data(second.business.id, second.booking.id, 'same-key'), 100); await expect(prisma.payment.findUnique({ where: { id: created.payment.id } })).resolves.toMatchObject({ businessId: first.business.id, bookingId: first.booking.id, amountMinor: 40n, currency: 'PYG', method: 'CASH', recordedByUserId: '11111111-1111-4111-8111-111111111111', status: 'RECORDED', idempotencyKey: 'same-key', requestFingerprint: 'fp:same-key:40' }); await expect(prisma.payment.count({ where: { businessId: second.business.id, idempotencyKey: 'same-key' } })).resolves.toBe(1); });

...
export const membershipRepositoryFake = {
  findByUserAndBusiness: (userId: string, businessId: string): Promise<UserBusinessMembership | null> => Promise.resolve(memberships.get(key(userId, businessId)) ?? null),
  findByUserId: (userId: string): Promise<UserBusinessMembership[]> => Promise.resolve([...memberships.values()].filter((item) => item.userId === userId).sort((left, right) => left.createdAt.getTime() - right.createdAt.getTime() || left.businessId.localeCompare(right.businessId))),
  create: ({ userId, businessId, role }: { userId: string; businessId: string; role: MembershipRole }): Promise<UserBusinessMembership> => { const now = new Date(); const item = UserBusinessMembership.create({ id: `membership-${memberships.size + 1}`, userId, businessId, role, createdAt: now, updatedAt: now }); memberships.set(key(userId, businessId), item); return Promise.resolve(item); },
};

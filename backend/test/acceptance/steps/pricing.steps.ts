...
Given('existe un Resource activo para seleccionar tarifas', function (): void { addResource(oneId); });
Given('existe una tarifa activa asignada al Resource seleccionable', async function (this: TopWorld): Promise<void> { addResource(oneId); await createPlan(this, [oneId]); });
When('listo el catálogo de tarifas del negocio', async function (this: TopWorld): Promise<void> { this.response = await request(this.app?.getHttpServer()).get(`/api/businesses/${businessId}/rate-plans`); });
When('listo las tarifas seleccionables para la estadía', async function (this: TopWorld): Promise<void> { this.response = await request(this.app?.getHttpServer()).get(`/api/businesses/${businessId}/rate-plans`).query({ resourceId: oneId, checkIn: '2026-08-10', checkOut: '2026-08-14' }); });
When('listo tarifas para el Resource de otro negocio', async function (this: TopWorld): Promise<void> { this.response = await request(this.app?.getHttpServer()).get(`/api/businesses/${businessId}/rate-plans`).query({ resourceId: threeId, checkIn: '2026-08-10', checkOut: '2026-08-14' }); });
When('listo tarifas fuera de la vigencia aprobada', async function (this: TopWorld): Promise<void> { this.response = await request(this.app?.getHttpServer()).get(`/api/businesses/${businessId}/rate-plans`).query({ resourceId: oneId, checkIn: '2026-09-01', checkOut: '2026-09-02' }); });
When('listo tarifas para el Resource no disponible', async function (this: TopWorld): Promise<void> { this.response = await request(this.app?.getHttpServer()).get(`/api/businesses/${businessId}/rate-plans`).query({ resourceId: threeId, checkIn: '2026-08-10', checkOut: '2026-08-14' }); });
Then('recibo un catálogo de tarifas vacío', function (this: TopWorld): void { assert.equal(this.response?.status, 200); assert.deepEqual(this.response?.body, []); });
Then('recibo la tarifa en el catálogo público', function (this: TopWorld): void { const body = this.response?.body as Array<Record<string, unknown>> | undefined; assert.equal(this.response?.status, 200); assert.ok(body); assert.equal(body.length, 1); assert.equal(body[0].id, planId(this)); assert.equal('props' in body[0], false); });
Then('recibo la tarifa archivada en el catálogo histórico', function (this: TopWorld): void { assert.equal(this.response?.status, 200); assert.equal(this.response?.body[0].status, 'ARCHIVED'); });
Then('recibo únicamente la tarifa seleccionable', function (this: TopWorld): void { assert.equal(this.response?.status, 200); assert.deepEqual(this.response?.body.map((plan: { id: string }) => plan.id), [planId(this)]); });

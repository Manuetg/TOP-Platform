...
describe('BusinessAuthorizationGuard', () => {
  const request = { params: { businessId }, authenticatedPrincipal: { userId } } as unknown as AuthenticatedRequest;
  ...
  beforeEach(() => { jest.resetAllMocks(); request.params = { businessId }; request.authenticatedPrincipal = { userId }; metadata.mockReturnValueOnce(false).mockReturnValueOnce({ parameter: 'businessId', roles: [] }); findByUserAndBusiness.mockResolvedValue(item(MembershipRole.VIEWER)); });
  ...
  it('aplica roles por Business', async () => {

...
import type { RatePlanResourceAssignmentLookup } from '../domain/rate-plan-resource-assignment.lookup';
...
@Injectable()
export class PrismaRatePlanRepository implements RatePlanRepository, RatePlanResourceAssignmentLookup {
  ...
  async listByBusinessId(businessId:string):Promise<RatePlan[]>{const rows=await this.prisma.ratePlan.findMany({where:{businessId},orderBy:[{name:'asc'},{id:'asc'}],include:{business:{select:{currency:true}},resources:{include:{resource:true},orderBy:{resourceId:'asc'}}}});return rows.map((row)=>this.map(row));}
  ...
  async isAssigned(ratePlanId: string, resourceId: string): Promise<boolean> { return Boolean(await this.prisma.ratePlanResource.findUnique({ where: { ratePlanId_resourceId: { ratePlanId, resourceId } }, select: { ratePlanId: true } })); }

...
export interface SeasonalRateRepository { create(data: CreateSeasonalRateData): Promise<SeasonalRate>; listByRatePlanId(ratePlanId: string): Promise<SeasonalRate[]>; listIntersectingRange(ratePlanId: string, checkIn: string, checkOut: string): Promise<SeasonalRate[]>; hasOverlap(ratePlanId: string, startDate: string, endDate: string): Promise<boolean>; hasOutsideValidity(ratePlanId: string, validFrom: string | null, validTo: string | null): Promise<boolean>; }

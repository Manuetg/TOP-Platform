...
export class PricingController {
  ...
  @Post() @BusinessAccess('businessId', Capability.PRICING_WRITE) @HttpCode(HttpStatus.CREATED) @ApiOperation({ summary: 'Creates an active rate plan with an optional resource assignment.' }) @ApiCreatedResponse({ type: RatePlanResponseDto }) @ApiBadRequestResponse() @ApiNotFoundResponse() @ApiConflictResponse()
  ...
  @Post(':ratePlanId/calculate') @BusinessAccess('businessId', Capability.PRICING_CALCULATE) @HttpCode(HttpStatus.OK) @ApiOperation({ summary: 'Calculates the nightly price for a Resource stay without persisting it.' }) @ApiOkResponse({ type: CalculatePriceResponseDto }) @ApiBadRequestResponse() @ApiNotFoundResponse() @ApiConflictResponse()
  ...
  @Post(':ratePlanId/calculate/override') @BusinessAccess('businessId', Capability.PRICING_OVERRIDE_CALCULATE) @HttpCode(HttpStatus.OK) @ApiOperation({ summary: 'Calculates a manual agreed total without persisting it.' }) @ApiOkResponse({ type: ManualPriceOverrideResponseDto }) @ApiBadRequestResponse() @ApiNotFoundResponse() @ApiConflictResponse()
  ...
  @Post(':ratePlanId/seasonal-rates') @BusinessAccess('businessId', Capability.PRICING_WRITE) @HttpCode(HttpStatus.CREATED) @ApiOperation({ summary: 'Creates a seasonal price rule for an active rate plan.' }) @ApiCreatedResponse({ type: SeasonalRateResponseDto }) @ApiBadRequestResponse() @ApiNotFoundResponse() @ApiConflictResponse()
  ...
  @Get(':ratePlanId/seasonal-rates') @BusinessAccess('businessId', Capability.PRICING_READ) @ApiOperation({ summary: 'Lists seasonal price rules in deterministic date order.' }) @ApiOkResponse({ type: [SeasonalRateResponseDto] }) @ApiBadRequestResponse() @ApiNotFoundResponse()
  ...
  @Patch(':ratePlanId') @BusinessAccess('businessId', Capability.PRICING_WRITE) @ApiOperation({ summary: 'Updates an active rate plan partially.' }) @ApiOkResponse({ type: RatePlanResponseDto })

...
describe('PricingController', () => {
  ...
  it('delegates manual override with exact scope and exposes its public DTO', async () => {
    const body = { resourceId: '44444444-4444-4444-8444-444444444444', checkIn: '2026-12-18', checkOut: '2026-12-19', agreedAmountMinor: 0, overrideReason: 'Cortesía' };
    overrideExecute.mockResolvedValueOnce({ businessId: ratePlan.businessId, ratePlanId: ratePlan.id, resourceId: body.resourceId, currency: 'PYG', checkIn: body.checkIn, checkOut: body.checkOut, nights: 1, pricingMode: 'MANUAL_OVERRIDE', suggestedAmountMinor: 1, agreedAmountMinor: 0, adjustmentAmountMinor: -1, overrideReason: body.overrideReason, suggestedBreakdown: [{ date: body.checkIn, amountMinor: 1, source: NightlyPriceSource.BASE }] });
    const result = await controller.override(ratePlan.businessId, ratePlan.id, body);
    expect(overrideExecute).toHaveBeenCalledTimes(1); expect(overrideExecute).toHaveBeenCalledWith({ businessId: ratePlan.businessId, ratePlanId: ratePlan.id, ...body });
    expect(result).toMatchObject({ pricingMode: 'MANUAL_OVERRIDE', agreedAmountMinor: 0 }); expect(result).not.toHaveProperty('props');
  });
  it('translates manual override validation errors to HTTP 400', async () => {
    overrideExecute.mockRejectedValueOnce(new InvalidManualPriceOverrideInputError('invalid'));
    await expect(controller.override(ratePlan.businessId, ratePlan.id, {} as never)).rejects.toBeInstanceOf(BadRequestException);
  });
  it.each([

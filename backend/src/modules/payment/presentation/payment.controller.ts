...
export class PaymentController {
  ...
  @ApiOperation({ summary: 'Registers a confirmed Payment with an idempotency key.' })
  @ApiCreatedResponse({ type: PaymentHistoryItemResponseDto, description: 'Public Payment representation. Internal idempotency metadata is never exposed.' })
  @ApiBadRequestResponse({ description: 'Invalid Payment input or missing idempotency key.' })
  ...
  @ApiNotFoundResponse({ description: 'The Business or Booking does not exist in the requested Business.' })
  async create(@Param('businessId') businessId: string, @Param('bookingId') bookingId: string, @Body() body: RegisterPaymentRequestDto, @Headers('idempotency-key') idempotencyKey: string | undefined, @Req() request: AuthenticatedRequest): Promise<PaymentHistoryItemResponseDto> {
    const principal = request.authenticatedPrincipal;
    if (!principal) throw new UnauthorizedException();
    try {
      const payment = await this.register.execute({ ...body, businessId, bookingId, idempotencyKey, actorUserId: principal.userId });
      return PaymentHistoryItemResponseDto.fromDomain(payment);
    }
    catch (error: unknown) {

...
describe('GetBusinessByIdUseCase', () => {
  ...
  it('retorna el negocio encontrado', async () => {
    ...
    const useCase = new GetBusinessByIdUseCase({ create: jest.fn(), findById, list: jest.fn(), update: jest.fn() });
  ...
  it('rechaza cuando el negocio no existe', async () => {
    const useCase = new GetBusinessByIdUseCase({ create: jest.fn(), findById: jest.fn().mockResolvedValue(null), list: jest.fn(), update: jest.fn() });

...
describe('ContactController', () => {
  const create = jest.fn(); const get = jest.fn(); const search = jest.fn(); const update = jest.fn(); const archive = jest.fn(); const controller = new ContactController({ execute: create } as never, { execute: get } as never, { execute: search } as never, { execute: update } as never, { execute: archive } as never);
  ...
  it('maps archive to public DTO and uses only the authenticated actor', async () => {
    archive.mockResolvedValueOnce(contact.archive());
    await expect(controller.archive(businessId, contactId, { authenticatedPrincipal: { userId: businessId } } as never)).resolves.toMatchObject({ status: 'ARCHIVED' });
    expect(archive).toHaveBeenCalledWith(businessId, contactId, businessId);
    archive.mockRejectedValueOnce(new ContactNotFoundError('El contacto no existe.'));
    await expect(controller.archive(businessId, contactId, {} as never)).rejects.toBeInstanceOf(NotFoundException);
  });

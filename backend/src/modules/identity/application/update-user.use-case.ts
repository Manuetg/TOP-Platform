...
export class UpdateUserUseCase {
  ...
  async execute(input: { id: string; email?: unknown }): Promise<User> {
    if (!uuid.test(input.id)) throw new InvalidUserUpdateError('El identificador del usuario no es válido.');
    if (typeof input.email !== 'string') throw new InvalidUserUpdateError('El email es obligatorio.');
    const normalizedEmail = input.email.trim().toLowerCase();
    if (!normalizedEmail) throw new InvalidUserUpdateError('El email es obligatorio.');
    if (!email.test(normalizedEmail)) throw new InvalidUserUpdateError('El email no es válido.');
    ...
    return this.repository.updateEmail(user.updateEmail(normalizedEmail));
  }
}

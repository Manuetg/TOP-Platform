...
describe('PrismaBookingRepository', () => {
  ...
  it('persists only CANCELLED status when cancelling a booking', async () => { booking.update.mockResolvedValueOnce(row({ status: BookingStatus.CANCELLED })); await expect(repository.markCancelled(bookingId)).resolves.toMatchObject({ id: bookingId, status: BookingStatus.CANCELLED }); expect(booking.update).toHaveBeenCalledWith({ where: { id: bookingId }, data: { status: BookingStatus.CANCELLED }, include: { resources: { orderBy: { resourceId: 'asc' } } } }); });

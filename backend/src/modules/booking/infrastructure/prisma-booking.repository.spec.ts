...
describe('PrismaBookingRepository', () => {
  ...
  it('atomically cancels only an allowed state and appends its event', async () => { booking.updateMany.mockResolvedValueOnce({count:1}); booking.findUniqueOrThrow.mockResolvedValueOnce(row({status:BookingStatus.CANCELLED})); await expect(repository.markCancelled(bookingId,businessId,null,'Motivo')).resolves.toMatchObject({status:BookingStatus.CANCELLED}); expect(bookingTimelineEvent.create).toHaveBeenCalled(); const eventCall=bookingTimelineEvent.create.mock.calls[0]?.[0]; expect(eventCall?.data).toMatchObject({businessId,bookingId,type:'BOOKING_CANCELLED',details:{reason:'Motivo'}}); });

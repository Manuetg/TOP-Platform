...
import type { Booking as PrismaBooking, BookingResource, Prisma } from '@prisma/client';
...
describe('PrismaBookingRepository', () => {
  const booking = { create: jest.fn<Promise<Row>, [unknown]>(), findFirst: jest.fn(), findMany: jest.fn(), update: jest.fn(), updateMany:jest.fn(), findUniqueOrThrow: jest.fn() }; const bookingResource = { deleteMany: jest.fn(), createMany: jest.fn() }; const bookingTimelineEvent={create:jest.fn<Promise<unknown>, [Prisma.BookingTimelineEventCreateArgs]>(),findMany:jest.fn()}; const transaction = { booking, bookingResource, bookingTimelineEvent }; const prisma = { $transaction: jest.fn((callback: (client: typeof transaction) => unknown) => callback(transaction)), booking, bookingResource, bookingTimelineEvent };
  ...
  it('creates an atomic empty draft and maps nullables', async () => {
    ...
    const [call] = booking.create.mock.calls; expect(call?.[0]).toMatchObject({ data: { businessId, status: 'DRAFT', resources: { create: [] } } }); expect(bookingTimelineEvent.create).toHaveBeenCalledWith({data:{businessId,bookingId,type:'BOOKING_CREATED',actorUserId:null,details:{}}});
  });
  ...
  it('atomically submits only a DRAFT and appends its event', async () => { booking.updateMany.mockResolvedValueOnce({count:1}); booking.findUniqueOrThrow.mockResolvedValueOnce(row({status:BookingStatus.PENDING})); await expect(repository.markPending(bookingId,businessId,null)).resolves.toMatchObject({status:BookingStatus.PENDING}); expect(booking.updateMany).toHaveBeenCalledWith({where:{id:bookingId,businessId,status:BookingStatus.DRAFT},data:{status:BookingStatus.PENDING}}); expect(bookingTimelineEvent.create).toHaveBeenCalled(); const eventCall=bookingTimelineEvent.create.mock.calls[0]?.[0]; expect(eventCall?.data).toMatchObject({businessId,bookingId,type:'BOOKING_SUBMITTED'}); });
  it('does not append Submit when the expected state changed',async()=>{booking.updateMany.mockResolvedValueOnce({count:0});await expect(repository.markPending(bookingId,businessId,null)).resolves.toBeNull();expect(bookingTimelineEvent.create).not.toHaveBeenCalled();});
  it('atomically cancels only an allowed state and appends its event', async () => { booking.updateMany.mockResolvedValueOnce({count:1}); booking.findUniqueOrThrow.mockResolvedValueOnce(row({status:BookingStatus.CANCELLED})); await expect(repository.markCancelled(bookingId,businessId,null,'Motivo')).resolves.toMatchObject({status:BookingStatus.CANCELLED}); expect(booking.updateMany).toHaveBeenCalledWith({where:{id:bookingId,businessId,status:{in:[BookingStatus.DRAFT,BookingStatus.PENDING,BookingStatus.CONFIRMED]}},data:{status:BookingStatus.CANCELLED}});expect(booking.findUniqueOrThrow).toHaveBeenCalledWith({where:{id:bookingId},include:{resources:{orderBy:{resourceId:'asc'}}}});expect(bookingTimelineEvent.create).toHaveBeenCalled(); const eventCall=bookingTimelineEvent.create.mock.calls[0]?.[0]; expect(eventCall?.data).toMatchObject({businessId,bookingId,type:'BOOKING_CANCELLED',details:{reason:'Motivo'}}); });
  it('does not append Cancel when the expected state changed',async()=>{booking.updateMany.mockResolvedValueOnce({count:0});await expect(repository.markCancelled(bookingId,businessId,null)).resolves.toBeNull();expect(bookingTimelineEvent.create).not.toHaveBeenCalled();expect(booking.findUniqueOrThrow).not.toHaveBeenCalled();});

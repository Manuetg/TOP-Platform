...
describe('ListBookingTimelineUseCase', () => {
  const find = jest.fn();
  const list = jest.fn();
  const repository = { create: jest.fn(), findByIdAndBusinessId: find, listByBusinessId: jest.fn(), update: jest.fn(), markPending: jest.fn(), markCancelled: jest.fn(), appendTimelineEvent: jest.fn(), hasBlockingBooking: jest.fn(), listBlockingBookings: jest.fn() };
  ...
  it.each([{ cursor: 'invalid' }, { limit: '0' }, { limit: '51' }])('rejects invalid pagination', async (input) => { await expect(useCase.execute({ businessId, bookingId, ...input })).rejects.toBeInstanceOf(InvalidBookingInputError); });

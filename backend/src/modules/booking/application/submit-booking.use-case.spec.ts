...
const booking = (values: Partial<Parameters<typeof Booking.create>[0]> = {}) =>
  Booking.create({
    id: bookingId,
    businessId,
...
describe('SubmitBookingUseCase', () => {
  ...
  const useCase = new SubmitBookingUseCase(
    ...
    { findByIdAndBusinessId: jest.fn(), findByBusinessAndCode: jest.fn(), listByBusinessId: jest.fn(), create: jest.fn(), update: jest.fn() },
  ...
  beforeEach(() => {
    ...
    findContact.mockResolvedValue({ id: contactId });
    findBooking.mockResolvedValue(booking());
    validate.mockResolvedValue({ valid: true, conflicts: [] });
    markPending.mockResolvedValue(booking({ status: BookingStatus.PENDING }));
  });

  it('submits a complete Draft after exact Availability validation', async () => {
    await expect(useCase.execute({ businessId, bookingId })).resolves.toMatchObject({
      id: bookingId,
  ...
  it.each([
    ...
    [booking({ checkInDate: new Date('2026-05-12'), checkOutDate: new Date('2026-05-10') }), InvalidBookingInputError],
    [booking({ status: BookingStatus.PENDING }), BookingNotDraftError],
  ] as const)('does not persist an invalid Draft state', async (current, error) => {
    findBooking.mockResolvedValueOnce(current);

    await expect(useCase.execute({ businessId, bookingId })).rejects.toBeInstanceOf(error);
  ...
  it('hides a missing or cross-tenant Booking and does not persist', async () => {
    ...
    await expect(useCase.execute({ businessId, bookingId })).rejects.toBeInstanceOf(
      BookingNotFoundError,
    );
  ...
  it('rejects Availability conflicts without changing the Booking status', async () => {
    validate.mockResolvedValueOnce({
      valid: false,
      conflicts: [{ resourceId, reasons: ['BOOKING_CONFLICT'] }],
    });
  ...
  it('validates identifiers before reading or persisting a Booking', async () => {
    await expect(useCase.execute({ businessId: 'invalid', bookingId })).rejects.toBeInstanceOf(
      InvalidBookingInputError,
    );
    await expect(useCase.execute({ businessId, bookingId: 'invalid' })).rejects.toBeInstanceOf(
      InvalidBookingInputError,
    );

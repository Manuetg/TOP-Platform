...
import {
  ...
  type AvailabilityRulesRepository,
} from '../domain/availability-rules.repository';
...
export class CheckAvailabilityUseCase {
  ...
  async execute(input: {
    ...
    const shortCircuit = deriveAvailability(
      resource.status,
      false,
      false,
    );
    ...
    const bookingFrom = this.addDays(
      range.from,
      -rules.bufferAfterDays,
    );

    const bookingTo = this.addDays(
      range.to,
      rules.bufferBeforeDays,
    );
  ...
  private range(input: {
    ...
    const from = parseAvailabilityDate(
      input.from,
      'La fecha inicial',
    );

    const to = parseAvailabilityDate(
      input.to,
      'La fecha final',
    );
  ...
  private async business(
    ...
    if (
      ...
      throw new AvailabilityBusinessUnavailableError(

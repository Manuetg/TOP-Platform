...
describe('CancelBookingUseCase', () => {
  const findBusiness = jest.fn(); const findBooking = jest.fn(); const markCancelled = jest.fn(); const appendTimelineEvent = jest.fn();
  const useCase = new CancelBookingUseCase({ findById: findBusiness, create: jest.fn(), list: jest.fn(), update: jest.fn() }, { create: jest.fn(), findByIdAndBusinessId: findBooking, listByBusinessId: jest.fn(), update: jest.fn(), markPending: jest.fn(), markCancelled, appendTimelineEvent, hasBlockingBooking: jest.fn(), listBlockingBookings: jest.fn() });
  ...
  it('keeps an already cancelled booking and appends a trace event', async () => { const cancelled = booking(BookingStatus.CANCELLED); findBooking.mockResolvedValueOnce(cancelled); await expect(useCase.execute({ businessId, bookingId, reason:'Cambio de planes' })).resolves.toBe(cancelled); expect(markCancelled).not.toHaveBeenCalled(); expect(appendTimelineEvent).toHaveBeenCalledWith(expect.objectContaining({businessId,bookingId,reason:'Cambio de planes'})); });

...
describe('CancelBookingUseCase', () => {
  ...
  it('rejects when a lost concurrent transition is not a cancellation',async()=>{findBooking.mockResolvedValueOnce(booking(BookingStatus.DRAFT)).mockResolvedValueOnce(null);markCancelled.mockResolvedValueOnce(null);await expect(useCase.execute({businessId,bookingId})).rejects.toBeInstanceOf(BookingCancellationNotAllowedError);});
  it('forwards a valid actor only for the real cancellation',async()=>{const actorUserId='33333333-3333-4333-8333-333333333333';await useCase.execute({businessId,bookingId,actorUserId});expect(markCancelled).toHaveBeenCalledWith(bookingId,businessId,actorUserId,undefined);});
});

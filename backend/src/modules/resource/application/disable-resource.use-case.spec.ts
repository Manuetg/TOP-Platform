...
import { DisableResourceUseCase } from './disable-resource.use-case';
...
const makeResource = (status = ResourceStatus.ACTIVE): Resource => Resource.create({ id: resourceId, businessId, name: 'Cabana', internalCode: 'CAB', description: 'Vista', capacityMinimum: 1, capacityMaximum: 4, capacityMaximumChildren: 2, status, sortOrder: 3, createdAt: new Date('2026-01-01'), updatedAt: new Date('2026-01-02') });

describe('DisableResourceUseCase', () => {
  ...
  const useCase = new DisableResourceUseCase({ findById: findBusiness } as never, resources);
  beforeEach(() => jest.clearAllMocks());

  it('cambia ACTIVE a OUT_OF_SERVICE y preserva sus datos', async () => {
    ...
    expect(result.status).toBe(ResourceStatus.OUT_OF_SERVICE);
    expect(result).toMatchObject({ id: resourceId, businessId, name: 'Cabana', internalCode: 'CAB', capacityMaximum: 4, createdAt: resource.createdAt });
    expect(result.updatedAt.getTime()).toBeGreaterThanOrEqual(resource.updatedAt.getTime());
    expect(update).toHaveBeenCalledTimes(1);
  });

  it('es idempotente para OUT_OF_SERVICE sin persistir ni cambiar timestamp', async () => {
    const resource = makeResource(ResourceStatus.OUT_OF_SERVICE);
    findBusiness.mockResolvedValue(makeBusiness());
    findResource.mockResolvedValue(resource);
  ...
  ])('rechaza identificadores invalidos sin consultar', async (input, error, message) => {

...
describe('CreateResourceUseCase', () => {
  ...
  const repository: ResourceRepository = { findByBusinessAndCode, findByIdAndBusinessId, listByBusinessId: jest.fn(), create, update: jest.fn() };
  const useCase = new CreateResourceUseCase({ findById, create: jest.fn(), list: jest.fn(), update: jest.fn() }, repository, { allocate: (_businessId, write) => write(10, {}) });

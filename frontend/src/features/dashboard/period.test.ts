...
describe("dashboard period UX", () => {
  ...
  ])("rejects invalid %s to %s", (from, to) =>
    expect(validateDashboardPeriod({ from, to })).not.toBeNull(),
  );
  it.each([
    ["2026-09-01", "2026-10-02"],
    ["2028-02-29", "2028-03-01"],
    ["2099-12-31", "2100-01-01"],
  ])("allows valid 31 days, leap and future dates", (from, to) =>
    expect(validateDashboardPeriod({ from, to })).toBeNull(),
  );
  it("proposes seven local calendar days across year boundary", () => {
    expect(initialDashboardPeriod(new Date(2026, 0, 2, 0, 15))).toEqual({
      from: "2025-12-27",
      to: "2026-01-03",
    });
  });

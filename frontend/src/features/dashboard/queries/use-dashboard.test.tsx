...
describe("useDashboard", () => {
  it("isolates Business and period and never keeps the previous metrics during a new request", async () => {
    ...
    const { result, rerender } = renderHook(
      (input: DashboardRequest) => useDashboard(input),
      { initialProps: options, wrapper },
    );
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(read).toHaveBeenCalledWith(options);
    expect(
      client.getQueryData(["dashboard", "a", options.from, options.to]),
    ).toEqual(data);
  ...
  it.each([
    ...
    { ...options, accessToken: "" },
    { ...options, to: "" },
    { ...options, to: "2026-11-01" },
  ])("does not request with missing context or invalid period", (input) => {
    const { wrapper } = setup();

...
export function validateDashboardPeriod({ from, to }: DashboardPeriod): string | null {
  const start = calendarDay(from);
  const end = calendarDay(to);
  if (start === null || end === null) return "Ingresa ambas fechas válidas con formato AAAA-MM-DD.";
  if (end <= start) return "Hasta debe ser posterior a Desde.";
  if ((end - start) / 86_400_000 > 31) return "Selecciona un período de hasta 31 días.";
  return null;
}

...
- **BR-027 — Pendiente configurable.** Tipo: Configurable. Estado: Aprobada. Dominios: Availability, Booking. Descripción: en la política efectiva por defecto Pendiente bloquea. Comportamiento: AVL-003 propone `pendingBlocksAvailability` por Negocio; su ausencia equivale a `true` y solo altera el bloqueo de `PENDING`, no los demás estados. Excepciones: ninguna.
